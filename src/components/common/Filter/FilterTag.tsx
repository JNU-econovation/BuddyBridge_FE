...
function Tag({
  ...
  categoryName: string;
  option: string;
  handleFilter: (category: string, option: string) => void;
}) {
  return (
    <span className={cn("tag")}>
      {option}
      <button className={cn("deleteBtn")} onClick={() => handleFilter(categoryName, option)}>
        X
      </button>
    </span>
  );
...
function FilterTagging({ searchParams, handleFilter }: FilterTagProps, ref: React.Ref<HTMLDivElement>) {
  ...
  return (
    <div className={cn("tagContainer")} ref={ref}>
      {disabilityOption &&
        disabilityOption.map((type) => <Tag categoryName="disabilityType" option={type} handleFilter={handleFilter} />)}
      {assistanceOption &&
        assistanceOption.map((type) => <Tag categoryName="disabilityType" option={type} handleFilter={handleFilter} />)}
    </div>
  );
}
